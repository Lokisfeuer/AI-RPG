...
def train(dataset, epochs=10, lr=0.001, val_frac=0.1, batch_size=25, loss=nn.CrossEntropyLoss(), **kwargs):
    ...
    model = NeuralNetwork(dataset.shape, len(dataset.classes))
    ...
    history = History(val_set, train_set, model, accuracy=accuracy, CrossEntropyLoss=cel,
                      **kwargs)  # define history object
    ...
    torch.save(model, f"model_{now}/model.pt")
...
def get_embedded():
    return torch.load(f"data_type_openai_embedded.pt")


if __name__ == "__main__":
    embedded_data = get_embedded()
    dataset = to_dataset(embedded_data)
    history, model = train(dataset=dataset, epochs=3, lr=0.0001)
    history.plot()
